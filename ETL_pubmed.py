...
def concatenate_datasets():
    ...
    with open(join(csv_file_path, 'dataset.csv'), 'w') as fout:
        for f in tqdm.tqdm(present_files):
            with open(join(csv_file_path, f)) as fin:
                header = next(fin)
                if not header_saved:
                    fout.write(header)
                    header_saved = True
...
@run_in_thread
def handle_references(idx, paper, df):
    ...
    if len(references):
        ...
        for reference in references:
            ...
            if ref_paper_joined is not None:
                if ref_pmid == "" and len(ref_paper_joined['cur_pmid']) and ref_paper_joined["cur_pmid"].values[
                    0] is not None and ref_paper_joined["cur_pmid"].values[0] != 0:
                    ref_pmid = ref_paper_joined["cur_pmid"].values[0]
                    obj['ref_pmid'] = str(ref_pmid)

                if ref_pmcid == "" and len(ref_paper_joined['cur_pmcid']) and ref_paper_joined["cur_pmcid"].values[
                    0] is not None and ref_paper_joined["cur_pmcid"].values[0] != 0:
                    ref_pmcid = ref_paper_joined["cur_pmcid"].values[0]
                    obj['ref_pmcid'] = str(ref_pmcid)
        ...
        new_df.to_csv(join(csv_file_path, 'joined', f'{str(uuid.uuid4())}.csv'), sep=';', index=False)
...
def join_dataset():
    ...
    df['cur_pmid'] = df['cur_pmid'].astype(int)
    df['cur_pmcid'] = df['cur_pmcid'].astype(int)

    tasks = [handle_references(idx, paper, df) for idx, paper in tqdm.tqdm(df.iterrows())]
    [t.join() for t in tasks]

    df.to_csv(open(join(csv_file_path, 'dataset.csv'), sep=';'))
